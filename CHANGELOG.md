...
- **多代理摄入管线**：摄入复杂文档时自动拆分为 Planner → 并行 Writer → Merger 三层代理。每个 Writer 只生成一个 wiki 页面（500-2000 token），大幅减少长输出幻觉。简单文档自动退化为原有单次生成。
- **Token 预算引擎**：查询时根据问题类型（精确/延续/探索）动态分配上下文预算，替代原有固定 60/20/5/15 字符比例。中文场景 token 利用率提升约 30%。
- **分层摘要体系**：摄入时自动生成每页摘要（200-300 字），存储在 `wiki/.summaries/`。查询时先用摘要覆盖更多页面，再展开 top-K 全文。
- **并行摄入队列**：批量导入支持最多 3 个文件同时处理（可配置至 8），批量导入速度提升 2-3 倍。

### 改进（Improvement）

- **摄入队列重构**：从单任务串行改为多任务并行，每个任务独立 AbortController，取消不影响其他任务。
- **查询分类器**：自动识别精确查询（引用具体股票/文件）、对话延续（短问题+长历史）、探索性查询（开放问题），分别优化上下文分配。
